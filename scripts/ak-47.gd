...
@export var TIME_TO_RELOAD : float = 3;
@export var SHOOT_TIME_SPEED : float = 0.1;
@export var INIT_HP : float = 100;
...
func discon():
	...
	for scene in get_tree().get_root().get_children():
		...
		if scene.name == "MainMenu":
			scene.show()
			continue
		if((scene.name != "GameManager") and (scene.name != "Settings")):
			scene.queue_free()
...
func makeShootSound():
	...
	sound.global_position = Barrel.global_position
...
func add_bullet():
	...
	#Trail scene
	var trail : Trail = load("res://trail.tscn").instantiate()
	...
	trail.create(bullet,Barrel.global_position)
func rotate_left():
